...
/**
 * Delete confirmation for a social link.
 *
 ...
 * is suppressible.
 *
 * Nothing references `social_links`, so this removes one row and cascades to
 * nothing.
 *
...
import { deleteSocialLinkAction } from "@/lib/actions/socials";

export function DeleteSocialLinkForm({
  socialLinkId,
  socialLinkLabel,
}: {
  socialLinkId: string;
  ...
  if (!isConfirming) {
    return (
      <div className="mt-5">
        ...
        >
          Delete social link
          <span className="sr-only"> {socialLinkLabel}</span>
        </button>
        {errorMessage ? (
  ...
  return (
    <form action={formAction} className="mt-5">
      ...
      <p role="alert" className="text-sm font-medium text-fg">
        Delete “{socialLinkLabel}” permanently?
      </p>
