...
/**
 * Delete confirmation for one of the robot's lines.
 *
 ...
 *
 * Nothing references `terminal_lines`, so this removes one row and cascades to
 * nothing.
 *
...
import { deleteTerminalLineAction } from "@/lib/actions/terminal-lines";

export function DeleteTerminalLineForm({
  lineId,
  lineText,
}: {
  lineId: string;
  ...
  if (!isConfirming) {
    return (
      <div className="mt-5">
        ...
        >
          Delete line
          <span className="sr-only"> “{lineText}”</span>
        </button>
        {errorMessage ? (
