...
import type { TerminalLineMutationData } from "@/lib/actions/terminal-lines";
import { CheckboxField, SelectField, TextField } from "@/components/form/field";

export interface TerminalLineFormValues {
  text: string;
  tone: string;
  status: string;
  position: number;
...
export const emptyTerminalLineValues: TerminalLineFormValues = {
  text: "",
  tone: "system",
  status: "",
  position: 0,
...
type TerminalLineAction = (
  previous: ActionState<TerminalLineMutationData>,
  formData: FormData,
) => Promise<ActionState<TerminalLineMutationData>>;

export function TerminalLineForm({
  action,
  lineId,
  initialValues,
  submitLabel,
}: {
  action: TerminalLineAction;
  lineId?: string;
  initialValues: TerminalLineFormValues;
  submitLabel: string;
}) {
  const fieldId = useId();
  const [state, formAction, isPending] = useActionState(action, idleState);
  const [values, setValues] = useState<TerminalLineFormValues>(initialValues);
  ...
  function update<K extends keyof TerminalLineFormValues>(
    key: K,
    value: TerminalLineFormValues[K],
  ) {
    ...
    <form action={formAction} className="mt-8 flex flex-col gap-10">
      ...
      >
        ...
        <TextField
          ...
          hint="One short console line. Keep it under about 80 characters — the terminal is narrow, and a longer line wraps into something that no longer reads as console output."
        ...
        <SelectField
          ...
          value={values.tone}
          options={[
            { value: "system", label: "System — the machine narrating itself" },
            { value: "speech", label: "Speech — the machine talking about you" },
          ]}
          errors={fieldErrors.tone}
          hint="Decides the line's colour. System lines read as the console doing work; speech lines read as the robot saying something."
        ...
        <TextField
          ...
          hint="Optional short badge printed right-aligned in green, like “ok”. Leave empty for an ordinary line — most lines have none."
