...
/**
 * Delete confirmation for a timeline entry.
 *
...
import { deleteTimelineEntryAction } from "@/lib/actions/timeline";

export function DeleteTimelineEntryForm({
  entryId,
  entryLabel,
  highlightCount,
}: {
    ...
    <form action={formAction} className="mt-5">
      ...
      <p role="alert" className="text-sm font-medium text-fg">
        ...
        {highlightCount > 0 ? (
          <>
            {" "}
            Its {highlightCount} highlight
            {highlightCount === 1 ? "" : "s"} will be deleted with it.
          </>
        ) : null}
      </p>
