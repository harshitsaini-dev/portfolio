...
/**
 * Education entry mutation Server Actions.
 *
 ...
 *
 * Education is a flat ordered entity: no owned child table and nothing
 * referencing it, so there is no aggregate write and no cascade to reason
...
/** Where every successful education mutation lands. */
const LIST_PATH = "/education";

const SAVE_CONFLICT =
  "This entry could not be saved. Please review the values and try again.";

/**
 * What a successful education mutation returns.
 *
 * One shape across create/update/delete so a form can be typed against a
 * single `ActionState`.
 */
export interface EducationMutationData {
  readonly id: string;
}
...
function toActionResult(error: unknown): ActionResult<never> {
  if (error instanceof NotFoundError) {
    return notFoundError("That education entry no longer exists.");
  }
  if (error instanceof ConflictError) return conflictError(SAVE_CONFLICT);
  console.error("[admin] education mutation failed", error);
  return failureError();
}
...
export async function createEducationEntryAction(
  _previous: ActionState<EducationMutationData>,
  formData: FormData,
): Promise<ActionState<EducationMutationData>> {
  await requireAdminIdentity();

  const parsed = educationEntryCreateSchema.safeParse(readPayload(formData));
  if (!parsed.success) {
  ...
  try {
    ...
    await repos.education.create(parsed.data);
  } catch (error) {
...
export async function updateEducationEntryAction(
  _previous: ActionState<EducationMutationData>,
  formData: FormData,
): Promise<ActionState<EducationMutationData>> {
  await requireAdminIdentity();

  const idResult = educationEntryIdSchema.safeParse(formData.get("id"));
  if (!idResult.success) {
    return validationError({ form: ["Missing education entry identifier."] });
  }

  const parsed = educationEntryUpdateSchema.safeParse(readPayload(formData));
  if (!parsed.success) {
    return validationError(toFieldErrors(parsed.error.issues));
  }
  ...
  try {
    ...
    const updated = await repos.education.update(idResult.data, parsed.data);
...
export async function deleteEducationEntryAction(
  ...
  try {
    const repos = await getAdminRepositories();
    // Nothing in the schema references `education`, so this removes exactly
    // one row and cascades to nothing.
    const deleted = await repos.education.delete(idResult.data);
    if (!deleted) {
      return notFoundError("That education entry no longer exists.");
    }
  } catch (error) {
