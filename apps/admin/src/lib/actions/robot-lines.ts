...
/**
 * Robot line mutation Server Actions.
 *
...
import {
  robotLineCreateSchema,
  robotLineIdSchema,
  robotLineUpdateSchema,
...
const LIST_PATH = "/robot-lines";

const GONE = "That line no longer exists.";

export interface RobotLineMutationData {
  readonly id: string;
}
 ...
 *
 * Repository messages describe internals ("robot line: update ...") and are
...
function toActionResult(error: unknown): ActionResult<never> {
  ...
  console.error("[admin] robot line mutation failed", error);
  return failureError();
}
...
export async function createRobotLineAction(
  _previous: ActionState<RobotLineMutationData>,
  formData: FormData,
): Promise<ActionState<RobotLineMutationData>> {
  await requireAdminIdentity();

  const parsed = robotLineCreateSchema.safeParse(readPayload(formData));
  if (!parsed.success) {
  ...
  try {
    ...
    await repos.robotLines.create(parsed.data);
  } catch (error) {
...
export async function updateRobotLineAction(
  _previous: ActionState<RobotLineMutationData>,
  formData: FormData,
): Promise<ActionState<RobotLineMutationData>> {
  await requireAdminIdentity();

  const idResult = robotLineIdSchema.safeParse(formData.get("id"));
  if (!idResult.success) {
    return validationError({ form: ["Missing line identifier."] });
  }

  const parsed = robotLineUpdateSchema.safeParse(readPayload(formData));
  if (!parsed.success) {
    return validationError(toFieldErrors(parsed.error.issues));
  }
  ...
  try {
    ...
    const updated = await repos.robotLines.update(idResult.data, parsed.data);
...
export async function deleteRobotLineAction(
  _previous: ActionState<RobotLineMutationData>,
  formData: FormData,
): Promise<ActionState<RobotLineMutationData>> {
  await requireAdminIdentity();
  ...
  try {
    const repos = await getAdminRepositories();
    // Nothing references `robot_lines`, so this removes exactly one row and
    // cascades to nothing.
    const deleted = await repos.robotLines.delete(idResult.data);
