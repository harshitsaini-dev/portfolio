 ...
 *
 * Deliberately NOT `generateMetadata` reading tool data. Phase 6 established
...
export default withAdminPage(async () => {
  ...
  // tools too, badged rather than omitted. Ordering comes from the
  // repository (position, then created_at) — never re-sorted here.
  const tools = await repos.tools.list();
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-3 text-sm text-fg-muted">
            {tools.length === 0
              ? "No tools yet."
              : `${tools.length} tool${tools.length === 1 ? "" : "s"}, in display order.`}
          </p>
        </div>
        <Link
          href="/tools/new"
      ...
      {tools.length === 0 ? (
        <div className="mt-10 rounded-lg border border-dashed border-strong bg-surface p-10 text-center">
          ...
          <p className="mx-auto mt-2 max-w-sm text-sm text-fg-muted">
            Tools you add appear here, ordered by display position. Hidden ones
            stay listed here but not on the public site.
          </p>
          <Link
            href="/tools/new"
        ...
        <div className="relative mt-8 overflow-x-auto">
          <table className="w-full min-w-[44rem] border-collapse text-left text-sm">
            <caption className="sr-only">
              All tools, ordered by display position
            </caption>
            <thead>
              <tr className="border-b border-subtle text-xs uppercase tracking-wider text-fg-muted">
                ...
                <th scope="col" className="py-3 pr-4 font-semibold">Link</th>
            ...
            <tbody>
              {tools.map((tool) => (
                <tr key={tool.id} className="border-b border-subtle">
                  ...
                  <td className="py-3 pr-4 text-fg-muted">{tool.position}</td>
