 ...
 *
 * Deliberately NOT `generateMetadata` reading certification data. Phase 6
...
export const metadata: Metadata = {
  title: "Certifications · Portfolio Admin",
};

export default withAdminPage(async () => {
  const repos = await getAdminRepositories();
  // No visibility filter: the CMS list is the admin view and shows hidden
  // certifications too, badged rather than omitted.
  const certifications = await repos.certifications.list();
        ...
        <div>
          ...
          <p className="mt-3 text-sm text-fg-muted">
            {certifications.length === 0
              ...
              : `${certifications.length} certification${certifications.length === 1 ? "" : "s"}, in display order.`}
          </p>
        </div>
        <Link
      ...
      {certifications.length === 0 ? (
        <div className="mt-10 rounded-lg border border-dashed border-strong bg-surface p-10 text-center">
          ...
          <p className="mx-auto mt-2 max-w-sm text-sm text-fg-muted">
            Certifications you add appear here, ordered by display position.
        ...
        <div className="relative mt-8 overflow-x-auto">
          <table className="w-full min-w-[44rem] border-collapse text-left text-sm">
            <caption className="sr-only">
              All certifications, ordered by display position
            </caption>
            <thead>
              <tr className="border-b border-subtle text-xs uppercase tracking-wider text-fg-muted">
                ...
                <th scope="col" className="py-3 pr-4 font-semibold">Issuer</th>
                <th scope="col" className="py-3 pr-4 font-semibold">Credential</th>
                <th scope="col" className="py-3 pr-4 font-semibold">Issued</th>
                <th scope="col" className="py-3 pr-4 font-semibold">Expires</th>
            ...
            <tbody>
              {certifications.map((certification) => (
                <tr key={certification.id} className="border-b border-subtle">
                  <th scope="row" className="py-3 pr-4 font-medium text-fg">
                    {certification.title}
                    {certification.isVisible ? null : (
                  ...
                  <td className="py-3 pr-4 text-fg-muted">
                    {certification.credentialUrl ? (
                      ...
                      >
                        {certification.credentialId ?? "Verify"}
                        <span className="sr-only">
                          {" "}
                          — opens {certification.title} in a new tab
                        </span>
                      </a>
                    ) : (
