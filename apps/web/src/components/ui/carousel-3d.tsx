...
/**
 * A 3D slideshow, for any list of cards.
 *
 * Written for the projects and generalised the day the skills section wanted
 * the same treatment. The alternative was a second carousel, and the second
 * one is always the one that quietly loses the accessibility work — the inert
 * off-stage slides, the measured stage height, the pauses that fire in the
 * right places. There is one carousel; it takes items and a way to draw them.
 *
...
export function Carousel3D<T>({
  ...
  items: readonly T[];
  /** Draws one card. The carousel positions it and never styles it. */
  renderItem: (item: T) => React.ReactNode;
  /** A stable key. Index would do until the list reorders, and then it would not. */
  getKey: (item: T) => string;
  /** Names the group for assistive technology, e.g. "Projects". */
  label: string;
  /** Singular, for the controls: "Previous project", "Next category". */
  itemNoun: string;
  /**
   * The layout when the carousel does *not* take over — on a phone, under
   * reduced motion, or with JavaScript off. It is a prop because that
   * fallback is the real page for those visitors, and two callers want
   * different column counts for it.
   */
  ...
  /** Bumped to re-arm the autoplay timer without advancing. See below. */
   ...
   *
   * A single item is a card, not a slideshow — arrows that cycle one thing
  ...
  */
  ...
  useEffect(() => {
    ...
    /*
      A modal over the page holds the carousel still.

      A card can open one — the skills cards open a panel listing every skill
      in a category — and advancing behind it moves the card that opened it
      off stage, where it becomes `inert`. Closing the panel then has nowhere
      to send focus back to, and a keyboard visitor is returned to the top of
      the document. Checking the DOM rather than threading a prop through: any
      caller's card may open a dialog, and none of them should have to tell
      the carousel about it.
    */
    const id = window.setTimeout(() => {
      // Checked when the timer fires rather than when it is set, and re-armed
      // rather than abandoned — a modal closes without telling this component,
      // and a carousel that stops for good the first time one opens is worse
      // than one that advances at the wrong moment.
      const held =
        document.querySelector("dialog[open]") !== null ||
        // Asked of the DOM rather than of `paused`, because this is the
        // moment a race is decided. Closing the panel focuses the button that
        // opened it, and `setPaused(true)` from that focus is a state update
        // that has not landed yet when a timer armed a full interval ago
        // fires a millisecond later. It advanced, the slide went `inert`, and
        // the focus that had just been restored fell to the top of the page.
        // `:focus-within` is true the instant focus moves.
        regionRef.current?.matches(":hover, :focus-within") === true;
      if (held) setRearm((n) => n + 1);
      else go(1);
    }, AUTOPLAY_MS);
    return () => window.clearTimeout(id);
  }, [enhanced, paused, active, rearm, go]);
  ...
  return (
    <div
      ...
      aria-label={enhanced ? label : undefined}
      onKeyDown={onKeyDown}
      /*
    ...
    >
        ...
        className={
          enhanced
            ? // `project-stage` supplies the perspective and the absolute
              ...
              // The class keeps its historical name: it is shared now, and
              // renaming it would touch a stylesheet for no behaviour.
              `project-stage ${gridClassName}`
            : gridClassName
        }
      >
      ...
      {enhanced ? (
        <div
          ...
          onPointerLeave={() => setPaused(false)}
        >
          <CarouselButton label={`Previous ${itemNoun}`} onClick={() => go(-1)}>
          ...
          */}
          ...
          <CarouselButton label={`Next ${itemNoun}`} onClick={() => go(1)}>
            <span aria-hidden="true">→</span>
          </CarouselButton>
        </div>
      ) : null}
