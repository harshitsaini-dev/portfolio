...
Page-view counts are collected by this site, stored in this site's D1, and
shown on the CMS dashboard. No third-party beacon.

A Cloudflare Web Analytics tag was written first and rejected by the owner on
sight: **"cloudflare pr nhi"** — the numbers describing this site's content
should sit beside that content, not on a vendor's dashboard behind a separate
login. That is the whole reason for the extra table, route and repository.

### What is stored, and why the shape is a privacy decision

There is no row per visit. `page_view_daily(day, path, views)` and
`referrer_daily(day, host, views)` are incremented with an UPSERT.

A row per visit is a timeline: a timestamp plus a path is enough to work out
that one person read three pages in sequence, and once that data exists it can
be joined against anything else. Counting into a day bucket destroys the
correlation **at write time**, so there is nothing to protect afterwards. It is
a guarantee by construction rather than by policy.

No IP, no user agent, no cookie, no identifier, no session. Two visits by one
person are indistinguishable from one visit by two, which is why the site needs
no consent banner: there is nothing personal to consent to.

The referrer is stored as a **host**, and reduced to one on the server rather
than in the browser. A client that could post the stored value could post
anything; parsing it server-side means what lands in the table is always
something a URL parser produced. Verified — a beacon carrying
`https://www.google.com/search?q=secret` stored `www.google.com` and dropped
the query.

### Why a beacon rather than counting renders

Counting inside the page render would be simpler and would be wrong: crawlers,
uptime checks and link-preview fetches all render the page and none are visits.
Requiring JavaScript to execute filters nearly all of that out.

`usePathname` re-fires it on client navigation, so a project opened from the
carousel is counted — the number most worth having, and the one a load-time
beacon would miss.

### The public write path is the app's only anonymous input

Everything else in `packages/schemas` validates input from an authenticated
admin. This validates an anonymous stranger, so `path` is regex-bounded to a
site-relative path — the check that stops `//evil.example` and
`https://…` from becoming a stored "page" of this site — and length-capped,
because `path` is half of a primary key and anything unbounded here is
unbounded in the database. Verified: `//evil.example/pwned` was rejected and
never reached the dashboard.

The endpoint answers 204 to everything, including its own failures. A counter
is not worth an error in a visitor's console.

## 2026-08-10 — Share cards, sitemap and structured data

### The origin is derived from the request, not configured

`getSiteOrigin()` reads the `host` header rather than an environment variable.

An environment variable is a second copy of a fact the request already carries,
and a second copy can disagree — this project has already paid for that once,
when `NEXT_PUBLIC_SITE_ORIGIN` was inlined at build time while the value only
existed at runtime and the build silently substituted `undefined`. Deriving it
also means canonical links, share cards and the sitemap follow the site to a
custom domain with no deploy and no variable to remember.

The scheme is decided rather than trusted — anything that is not `localhost` is
HTTPS — so no attacker-controlled header chooses it.

### The share image is the owner's portrait

Not a purpose-built Open Graph card. A portfolio link is nearly always shared
as "here is this person", the face is the most useful large image the site has,
and it works today rather than after an upload nobody has made. Project pages
use their cover, falling back to the first gallery image; both are pictures the
editor already chose.

### The sitemap is dynamic

`sitemap.ts` is statically generated by default, which would freeze the project
list into the bundle — publishing from the CMS would add a page the sitemap
never mentions until the next deploy, in a site whose entire point is that
content changes without one. `force-dynamic` costs a query per crawl.

It lists published projects only, from the same status filter the page
enforces. A sitemap advertising a draft publishes a URL that answers 404, which
teaches a crawler the site lies about what it has.

### JSON-LD needs the CSP nonce

`application/ld+json` is data the browser never executes, but `script-src`
applies to every `<script>` element regardless of type. Without the nonce it is
dropped silently and the structured data is simply absent — the failure looks
like nothing at all.

### `global-error.tsx`, because `error.tsx` could not have caught the outage

`app/error.tsx` renders inside the root layout, so it only catches what is
thrown below it. The 2026-08-09 outage was thrown *by* the layout —
`getSiteContent()` fails in its `generateMetadata` — so the layout never
produced a tree for a boundary to live in. The class of failure most worth
catching was exactly the class `error.tsx` cannot see.

`global-error.tsx` replaces the layout, so it styles itself with inline
declarations rather than `globals.css`: a fallback that depends on the thing it
is falling back from is not a fallback.

Neither page prints `error.message`. A thrown database error names tables and
columns, and a visitor cannot act on any of it. `digest` is shown because it is
the one value that links a report to the log, and it is a hash with no content.

### The 404's back-link is a plain `<a>`

`next/link` navigates on the client, re-entering the router state that just
threw. A document load rebuilds everything, which is the point of an escape
hatch. The Next lint rule is disabled on that line with the reason inline.

### Missing logos get a monogram

Logos are optional, and entries without one previously rendered nothing — so
names started at two different positions down the same column, which reads as a
layout bug rather than as missing content. A monogram occupies the space a logo
would. It is `aria-hidden`, because the organisation's name is printed in full
beside it. The tint is a fixed accent rather than a hash of the name: generated
hues cannot hold a contrast guarantee across both themes.
